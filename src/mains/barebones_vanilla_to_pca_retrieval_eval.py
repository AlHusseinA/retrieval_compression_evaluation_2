...
from retrieval.run_retrieval_evaluation import run_retrieval_evaluation_PCA
import numpy as np
import random
...
def main():
    ...
    dir_retrieval_pca = "/home/alabutaleb/Desktop/confirmation/Retrieval_eval_PCA_evaluation"
    ...
    # ##############################################################################    
    COMPRESSED_VECTOR_DIMS_PCA = [1024, 512, 256, 128, 64, 32, 16, 8]


    ####################################################################################
    results_PCA = run_retrieval_evaluation_PCA(vanilla_model, trainloader_cub200, testloader_cub200, COMPRESSED_VECTOR_DIMS_PCA, dir_retrieval_pca, dataset_name, num_classes_cub200, batch_size, lr, top_k, device)

    save_retrieval_results(results_PCA, f"{dir_retrieval_pca}/results_dic_all_PCA_retrieval_eval.pkl")
    plot_metrics(results_PCA, top_k, COMPRESSED_VECTOR_DIMS_PCA, dir_retrieval_pca)
    plot_metrics_logarithmic(results_PCA, top_k, COMPRESSED_VECTOR_DIMS_PCA, dir_retrieval_pca)
    save_retrieval_results(results_PCA, f"{dir_retrieval_pca}/results_dic_PCA_resnet50.pkl")
